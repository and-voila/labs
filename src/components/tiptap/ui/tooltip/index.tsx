...
const ShortcutKey = ({ children }: { children: string }): JSX.Element => {
  const className =
    'inline-flex items-center justify-center w-5 h-5 p-1 text-[0.625rem] rounded font-semibold leading-none border text-foreground border-b-2';
...
export const Tooltip = ({
  ...
  const renderTooltip = useCallback(
    (attrs: TippyProps) => (
      <span
        className="z-[999] flex items-center gap-2 rounded-lg bg-card px-2.5 py-1 shadow-sm"
        tabIndex={-1}
